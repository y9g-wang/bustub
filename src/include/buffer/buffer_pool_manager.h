...
class BufferPoolManager {
 ...
 private:
  ...
  Page *GetPageByPageID(page_id_t page_id) {
    for (size_t i = 0; i < pool_size_; i++) {
      if (pages_[static_cast<int>(i)].GetPageId() == page_id) {
        return &pages_[static_cast<int>(i)];
      }
    }
    return nullptr;
  }

  void MustEvictPage(Page *page) {
    if (!page) {
      return;
    }
    if (page->pin_count_ > 0) {
      throw Exception("pin count is more than zero");
    }
    if (page->IsDirty()) {
      auto promise = disk_scheduler_->CreatePromise();
      auto future = promise.get_future();
      disk_scheduler_->Schedule(DiskRequest{true, page->GetData(), page->page_id_, std::move(promise)});
      if (!future.get()) {
        throw Exception("failed to write to disk");
      }
    }

    // clean up replacer_
    frame_id_t frame_id = page_table_[page->page_id_];
  ...
  Page *GetPageByFrameID(frame_id_t frame_id) {
    for (auto [k, v] : page_table_) {
      if (v == frame_id) {
        return GetPageByPageID(k);
      }
    }
    return nullptr;
  }
  ...
  bool GetFreeFrame(frame_id_t *frame_id) {
    ...
    if (!page) {
      throw Exception("failed to find page_id in bpm");
    }
    if (page->pin_count_) {
      throw Exception("pin count is more than zero");
    }
